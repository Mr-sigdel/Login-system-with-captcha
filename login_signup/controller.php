<?php
    ...
    if (isset($_POST['signup'])) {
        ...
        // count erros
        if (count($errors) === 0) {
            $insertQuery = "INSERT INTO `signup` (`firstName`,`lastName`,`email`,`password`,`gender`,`code`,`status`)
            ...
            if ($insertInfo) {
                ...
                $sender = 'From: abhyddhy123@gmail.com';
    ...
    if (isset($_POST['verify'])) {
        ...
        $otp_query = "SELECT * FROM signup WHERE code = $otp";
        ...
        if (mysqli_num_rows($otp_result) > 0) {
            ...
            $update_query = "UPDATE signup SET status = '$update_status' , code = $update_code WHERE code = $fetch_code";
...
if($captcha==$_SESSION["CODE"]){
        ...
        if (mysqli_num_rows($email_check) > 0) {
            $passwordQuery = "SELECT * FROM signup WHERE `email` = '$email' AND `password` ='$password';";
            $password_check = mysqli_query($conn, $passwordQuery);
            echo mysqli_num_rows($password_check);
    ...
    if (isset($_POST['forgot_password'])) {
        ...
        if ($emailCheckResult) {
            ...
            if (mysqli_num_rows($emailCheckResult) > 0) {
                $code = rand(999999, 111111);
                $updateQuery = "UPDATE signup SET code = $code WHERE email = '$email'";
                $updateResult = mysqli_query($conn, $updateQuery);
                if ($updateResult) {
                    $subject = 'Email Verification Code';
                    $message = "our verification code is $code";
                    $sender = 'From: abhyddhy123123@gmail.com';
...
if(isset($_POST['verifyEmail'])){
    ...
    $verifyQuery = "SELECT * FROM signup WHERE code = $OTPverify";
    $runVerifyQuery = mysqli_query($conn, $verifyQuery);
    if($runVerifyQuery){
        if(mysqli_num_rows($runVerifyQuery) > 0){
            $newQuery = "UPDATE signup SET code = 0";
...
if(isset($_POST['changePassword'])){
    ...
    if (strlen($_POST['password']) < 8) {
        ...
        if ($_POST['password'] != $_POST['confirmPassword']) {
            ...
            $password = hash("sha256",$_POST["password"]);
            $updatePassword = "UPDATE signup SET password = '$password' WHERE email = '$email'";
